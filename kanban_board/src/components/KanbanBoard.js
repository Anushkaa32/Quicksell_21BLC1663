...
const KanbanBoard = ({ tickets, users, grouping, sorting }) => {
  // Define columns for status-based grouping
  const statusColumns = {
    backlog: 'Backlog',
    todo: 'Todo',
  ...
  // Define columns for priority-based grouping
  ...
  // Function to sort tickets based on the sorting value (priority or title)
  const sortTickets = (tickets) => {
    return tickets.sort((a, b) => {
      if (sorting === 'priority') {
        return b.priority - a.priority; // Sort by priority in descending order
      } else if (sorting === 'title') {
        return a.title.localeCompare(b.title); // Sort by title in ascending alphabetical order
      }
      return 0;
    });
  };

  // Group tickets by status (default behavior)
  const groupByStatus = (tickets) => {
    ...
    Object.keys(grouped).forEach((status) => {
      grouped[status] = sortTickets(grouped[status]); // Apply sorting within each status group
    });
  ...
  const groupByUser = (tickets) => {
    ...
    tickets.forEach((ticket) => {
      if (grouped[ticket.userId]) {
        grouped[ticket.userId].push(ticket); // Assign ticket to the user's group
      }
    });

    // Sort tickets within each user group
    Object.keys(grouped).forEach((userId) => {
      grouped[userId] = sortTickets(grouped[userId]); // Apply sorting within each user group
    });
  ...
  const groupByPriority = (tickets) => {
    ...
    tickets.forEach((ticket) => {
      grouped[ticket.priority].push(ticket); // Group tickets by their priority level
    });

    // Sort tickets within each priority group
    Object.keys(grouped).forEach((priority) => {
      grouped[priority] = sortTickets(grouped[priority]); // Apply sorting within each priority group
    });

    return grouped;
  };

  // Function to render the priority icon based on the column key
  const renderPriorityIcon = (priorityKey) => {
    switch (priorityKey) {
      case '4': // Urgent
        return (
      ...
      case '1': // Low
        return (
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M3.5 8H2.5C1.94772 8 1.5 8.44772 1.5 9V13C1.5 13.5523 1.94772 14 2.5 14H3.5C4.05228 14 4.5 13.5523 4.5 13V9C4.5 8.44772 4.05228 8 3.5 8Z" fill="#5C5C5E" fill-opacity="0.4" />
  ...
  // Function to render the status icon based on the column key
  ...
  // Determine how to group the tickets based on the selected grouping (status, user, or priority)
  let groupedTickets;
  let columns;

  if (grouping === 'user') {
    groupedTickets = groupByUser(tickets);
    columns = users.reduce((acc, user) => {
      acc[user.id] = user.name;
      return acc;
    }, {}); // Map user IDs to user names for column headers
      ...
      {Object.keys(columns).map((columnKey) => (
        <div key={columnKey} className="kanban-column">
          <div className="kanban-column-header">
            {grouping === 'priority' && renderPriorityIcon(columnKey)}
            {grouping === 'status' && renderStatusIcon(columnKey)}
            <h3>{columns[columnKey]}</h3>
            <span className="ticket-count">({groupedTickets[columnKey]?.length || 0})</span>
          ...
          {groupedTickets[columnKey] &&
            groupedTickets[columnKey].map((ticket) => <TicketCard key={ticket.id} ticket={ticket} />)}
        </div>
      ))}
    </div>
  );
